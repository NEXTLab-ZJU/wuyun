        ...
        for notes_list_idx in range(len(notes_list)):
            ...
            # 中断音
                ...
                if len(single_rhythm_seg) == 1:
                    ...
                    #     # 单一骨干音 [骨干音] 并入上一小节
                    #     if len(rhythm_seg_notes_list) != 0:
                    #         idx = len(rhythm_seg_notes_list) - 1
                    #         # 向上寻找，直到非中断音
                    #         while idx > 0:
                    #             if rhythm_seg_notes_list[idx-len(rhythm_seg_notes_list)][0].priority != 5:
                    #                 rhythm_seg_notes_list[idx-len(rhythm_seg_notes_list)].append(single_rhythm_seg[0])
                    #                 break
                    #             else:
                    #                 idx -= 1
                    #         # 如果上面全是中断音，则将该骨干音放入下一小节
                    #         if idx == 0:
                    #             continue
                    #     # TODO 单一骨干音 [骨干音] 无上一小节
                    #     else:
                    #         continue
                    #         # rhythm_seg_notes_list.append(single_rhythm_seg)
                    # # 如果是装饰音
                    ...
                    if len(single_rhythm_seg) != 0:
                        if has_skeleton_note(single_rhythm_seg):
                            ...
                            if len(rhythm_seg_notes_list) != 0:
                                ...
                                rhythm_seg_notes_list.append(single_rhythm_seg)
                single_rhythm_seg = []
                # rhythm_seg_notes_list.append([note])
...
if __name__ == '__main__':
    rootdir = '/Users/xinda/Documents/Github/MDP/paper_vis/data_version2/2_quantization_HeyJude.mid'
    ...
    RPS = m.get_RPS_List(rhythm_cell_seg_notes_list)
    # print(RPS)
