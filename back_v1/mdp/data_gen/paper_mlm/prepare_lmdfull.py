...
from utils.midi_common_process.dedup import dedup
...
raw_dataset_path_root = '/Users/xinda/Documents/Github/MDP/data/raw/research_dataset/lmd_full'
dst_dataset_path_root = os.path.join(root, 'data_gen/paper_mlm/exp_lmdfull')

dataset_name = 'lmdfull'
...
def file_count(src_dir):
    count = glob(f"{src_dir}/**/*.mid", recursive=True)
    return len(count)

def melody_preprocessing():
    # count midi files
    # print(f"dataset = {dataset_name}, melody number = {file_count(raw_dataset_input_dir_path)}")
